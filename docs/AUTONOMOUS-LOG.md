...
2. **Schermata dettaglio/modifica/elimina evento** (tap su un evento).
3. **Login PKCE** (sostituisce la dev key).
4. **Integrazione CalendarContract** (Google/Samsung Calendar) — requisito core "non standalone".
5. **Rifinitura**: deprecazione `hiltViewModel`, tocchi Material 3 Expressive, stati loading/errore, più test.
